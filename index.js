...
client.on(Events.InteractionCreate, async (interaction) => {
    try {
        ...
        // Segurança básica
        if (!approvalData) return interaction.editReply({ content: "❌ Expired.", components: [] });
        if (approvalData.processed) return interaction.editReply({ content: `⚠️ Already processed by ${approvalData.processedBy}.`, components: [] });
        ...
        // --- LIBERAR PRODUTO NA FILA IMEDIATAMENTE ---
        try {
        ...
        try {
            ...
            if (remaining > 0) {
                // AINDA TEM TENTATIVAS: Mantém o objeto pendingApprovals vivo para a próxima tentativa
                await dm.send({ 
                ...
                // IMPORTANTE: Marca como processado para evitar cliques duplos nos botões antigos
                approvalData.processed = true; 
                approvalData.processedBy = interaction.user.username;
                await invalidateApprovalButtons(approvalData, `❌ Denied by ${interaction.user.username}. Customer notified.`);
                
                // NÃO DELETAMOS O PENDINGAPPROVALS AQUI! 
                // Precisamos dele para saber quantas tentativas o cliente já usou quando ele clicar em "Send New Receipt"
                
            } else {
                // TENTATIVAS ESGOTADAS: Fim da linha
                await dm.send({ 
...
clientSession[interaction.user.id] = {
    ...
    try {
        ...
        // CORREÇÃO CRÍTICA: Preserva o número de tentativas anteriores se elas existirem
        let currentAttempts = 0;
        if (pendingApprovals[interaction.user.id]) {
            currentAttempts = pendingApprovals[interaction.user.id].attempts || 0;
        }

        // Atualiza a sessão do cliente mantendo o contador de falhas
        clientSession[interaction.user.id].awaitingReceipt = { 
            productId: session.product.id, 
            store: session.product.store, 
            paymentMethod: "Lindens", 
            attempts: currentAttempts // <-- AQUI ESTAVA O ERRO (antes era 0)
        }; 
